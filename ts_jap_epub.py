...
def replace_chars_in_html(html_content, mapping):
    ...
    # 第一步：簡体 → 繁体
    html_content = cc.convert(html_content)

    # 第二步：繁体 → 日本語用字
    for left, right in mapping.items():
        count = html_content.count(left)
        if count > 0:
            replace_count += count
            html_content = html_content.replace(left, right)
    return html_content
        ...
        for file in files:
            if file.endswith((".html", ".xhtml")):
                ...
                with open(full_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                new_content = replace_chars_in_html(content, char_map)
...
if input_path:
    output_path = input_path.replace('.epub', '_modified.epub')
    count = process_epub(input_path, output_path)
    print(f"✅ 修正されたファイルが生成されました：{output_path}")
